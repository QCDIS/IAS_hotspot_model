...
#for(sel.sen in c(2:7)){
 ...
 # scenario <- dataset_scenarios[[sel.sen]]
  #  outdir <- paste(download_dir,"/datalayer.tiff/",scenario,"/",sep="")

for(sel.sen in c(3:7)){
  scenario <- dataset_scenarios[[sel.sen]]
  for(dec in dec.vec[c(2,3)]){
    print(paste("scenario: ", scenario, dec))
    # indir <- paste(dir,"datalayer.nc/",dataset_scenarios[[1]][1],"/",sep="")
    # outdir <- paste(dir,"datalayer.nc/",scenario,dec,"/",sep="")


    stackpath <- paste(download_dir,"rasterstacks/",scenario,dec,"/",sep="")
    rasterpath <- paste(download_dir,"datalayer.nc/",scenario,dec,sep="")

    if (!dir.exists(stackpath)) dir.create(stackpath, recursive = TRUE)
    print(paste("stackpath: ", stackpath))
    # We must have a raster stack in order to do focal interpolation
