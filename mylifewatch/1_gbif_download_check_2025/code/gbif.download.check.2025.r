...
species_file <- "/mnt/inputs/0018230-250310093411724.csv"# Arter från Matthias Mars 2024
...
variables = c(args$bio_oracle_variables)
dir <- tempdir()
download_layers(dataset_id, variables, constraints, fmt = "csv", directory = dir)

input <- read.delim(paste(path,species_file,sep=""),header =T,sep="\t",
                    na = c("", "NA"))#[ 1:1000,]
...
xlim <- c(-180,180)
ylim <- c(-40, 80)testplot.cleanput.mar2024.jpg

#for(s in all.species[c(8,9)]){
  for(s in all.species){
    
jpeg(test_plot_cleanput_2024, width = 18*( xlim[2] - xlim[1]),height = 18*( ylim[2] - ylim[1]), pointsize = 4)
plot(world1$geometry, xlim = xlim, ylim = ylim, col = "light grey")
