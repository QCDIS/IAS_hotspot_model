...
rastermappath <- "/mnt/outputs/predict.rasters"

ICESpath <- "/mnt/outputs/ICES_areas"
ICESecopath <- "/mnt/outputs/ICES_ecoregions"

# read in the map
shape2 <- readOGR(dsn="/mnt/inputs/NUTS_shapefile",layer="nutsByHand")
...
for(Species in Data.table$species){
 ...
 # map[is.na(map)]<- -0.1
  png(paste(Plotpath_output,"/",Species, ".logprob.Eur.png",sep=""),  width = 180, height = 180, units = "mm", res=1200)
  ...
  dev.off()
  ### same for Sweden
  png(paste(Plotpath_output,"/",Species, ".logprob.Swe.png",sep=""),  width = 180, height = 180, units = "mm", res=1200)
...
png(paste(Plotpath_output,"/mean.all.eur", ".png",sep=""),  width = 180, height = 180, units = "mm", res=1200)
plot(logpredvar,col=colorsBr, breaks=brk.log)
plot(shape2,add = TRUE, xlim=xlim, ylim=ylim, border = 1, lwd = 0.1)

dev.off()
png(paste(Plotpath_output,"/mean.all.Swe", ".png",sep=""),  width = 180, height = 180, units = "mm", res=1200)
...
#Dessa kartor �r nog viktigast. Jag undrar om vi kan f� rasterfiler f�r
#2. medelv�rdet av Log-sannolikheten
#3. summerad Log-sannolikheten
#4. antal arter �ver 50%-tr�skel delad upp efter 2 grupperingar (group:Environment, group:Salinity) enligt bifogat masterfile (IASmodellng_speciesList)
#
...
newfile <- paste(Plotpath_output,"/XX",content3,".EUR.png",sep="")
...
newfile <- paste(Plotpath_output,"/XX",content3,".SWE.png",sep="")
png(newfile,  width = 180, height = 180, units = "mm", res=1200)
