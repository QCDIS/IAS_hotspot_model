...
SpeciespathRaw <- "/mnt/inputs/species" # Path to species data
# Path where speciesdata with extracted environmental variables will be placed
Speciespath <- paste(SpeciespathRaw,suffix, sep ="")
data_table <- "/mnt/inputs/data.table.all.csv" # Path to the data table
Stackpath <- "/mnt/inputs/processlayers" # Folder where the rasterstacks are stored
iterations <- "/mnt/input/Iterations"
ICESecopath <- "/mnt/inputs/ICES_ecoregions" # Path to ICES ecoregions shapefiles
trafficpath <- "/mnt/inputs/traffic/2016_All_shiptypes_AIS_Shipping_Density"
roc <- "/mnt/inputs/ROC" # Folder where ROC curves are stored

plots <- "/mnt/outputs/Plots" # where plots should be stored
Models <- "/mnt/outputs/Models" # the modelpath is where the models from random forest is saved. The files will also contain predictions from the cross validation,
results <- "/mnt/output/Results"
mappath <- "/mnt/output/Maps"
RF_indata <- "/mnt/outputs/RF_indata"
rastermappath <- "/mnt/output/Rastermaps"
# ICESpath <- "/mnt/inputs/ICES" # Path to ICES shapefiles
species_stats = "/mnt/outputs/species_stats" # Path to species stats


#The "outpath" contains the .csv files with species data and extracted environmental variables
Outpath <- paste(RF_indata,suffix, sep ="")
...
#Iterations.path <- paste(path,"Iterations.no.chlora", sep ="/")
Iterations.path <- paste(iterations,suffix, sep ="")


#ROC.path <- paste(path,"ROC.no.chlora", sep ="/")
ROC.path <- paste(roc,suffix, sep ="")


#Plotpath <- paste(path, "Plots.no.chlora" ,sep="/")
Plotpath <- paste(plots,suffix,sep="")


# used as input when ROC curves are prpares
Modelpath <-  paste(Models,suffix, sep ="")
...
resultpath <-  paste(results ,suffix,sep="")
selectionspath <-  paste(results,suffix ,sep="")
...
Data.table <- read.csv(data_table,header=TRUE, sep=",",stringsAsFactors = F)
...
write.csv2(as.data.frame(stats),file =  paste(species_stats,suffix,".csv", sep=""))
#print(species)
###
...
write.csv2(as.data.frame(all.AUC),file =  paste(path,"/all.AUC",suffix,".csv", sep=""))
##################################################################
## plot maps
##################################################################
