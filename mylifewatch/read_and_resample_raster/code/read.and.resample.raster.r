...
path = list(
  stackpath = "/mnt/outputs/rasterstacks",
  raster_stack_plot = "/mnt/outputs/rasterstacks/raster_stack_plot.png"
)

rasterpaths <- c("/mnt/inputs/Present_Benthic_Avg",
                 "/mnt/inputs/Present_Surface")
...
print(paste("Temporary directory for raster processing:", tmp))
print(paste("Temporary directory for resampled rasters:", tmp2))

print(paste("rasterpaths:", rasterpaths))

#find all layers in the different folders. Copy to temporary directory for processing.
for(mypath in rasterpaths){
  print(paste("Processing rasters from:", mypath))
  lista.ras<- Sys.glob(paste(mypath,"*.tif.zip",sep="/"))
  for(file in lista.ras){
    unzip(file,exdir =tmp)
  }
  print(paste("lista of rasters found:", length(lista.ras)))
  print(lista.ras)
}
...
cat("Count of raster files found:", length(lista.ras2), "\n")
...
if (!dir.exists(path$stackpath)) {
  dir.create(path$stackpath, recursive = TRUE)
}
#
# # save the plot
# png(filename = path$raster_stack_plot)
# plot(mean(var))
# dev.off()
...
save(var, file=paste(path$stackpath,"globalStack.rda", sep="/"))
...
save(var2, file=paste(path$stackpath,"europeStack.rda", sep="/"))
